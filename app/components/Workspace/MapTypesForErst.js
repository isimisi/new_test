...
const MapTypesForErst = (props) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" maxWidth="md" fullWidth>
      <DialogTitle id="form-dialog-title">Hov, vi mangler lidt info, før vi kan trække fra CVR</DialogTitle>
      <DialogContent>
        <Typography variant="h6">
            Elementer
        </Typography>
        {Object.keys(initErstTypes.nodes).map((node) => (
          <Grid style={{ marginTop: 5 }} key={node} container alignItems="flex-start" direction="row" justify="space-between">
            <Grid item md={6}>
        ...
        {Object.keys(initErstTypes.edges).map((edge, index) => (
          <Grid style={{ marginTop: 5 }} key={edge} container alignItems="flex-start" direction="row" justify="space-between">
            <Grid item md={6}>
              <Typography display="inline">
           Hvilken type relation skal
            ...
            <Grid item md={6}>
              <CreatableSelect
                styles={selectStyles(initErstTypes.edges.length - 1 === index ? 'relative' : 'absolute')}
                placeholder={'Vælg type for ' + edge}
                options={relationships.toJS().map(n => ({ value: n.label, label: n.label }))}
                value={erstTypes.edges[edge]
                  && relationships.toJS().map(n => n.label).includes(erstTypes.edges[edge])
                  && {
