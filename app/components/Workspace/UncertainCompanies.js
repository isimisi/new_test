...
function UncertainCompanies(props) {
  const { uncertainCompanies, getAsyncOptions, changeCompany } = props;
  ...
  return (
    <>
      <DialogContentText>
        Vi har fundet en fejl! Der er et eller flere udenlandske selskaber, som vist ikke
        er registreret korrekt. Ofte er det fordi, at selskabet ikke er registreret med
        sit fulde navn i fx CVR. Vi har forsøgt at foretage et match for dig. Se og ret
        forslagene her.
      </DialogContentText>
      ...
      {Object.keys(uncertainCompanies).map((company) => (
        <Grid
          style={{ marginTop: 5 }}
          key={uncertainCompanies[company].id}
          container
          alignItems="center"
          direction="row"
