...
function Timeline() {
   ...
   const onInit = (_reactFlowInstance: ReactFlowInstance) => {
      ...
      dispatch(
         showTimeline(user, id as string, setMetaOpen, _reactFlowInstance)
      );
   };
   ...
   const onSaveDocument = async (
      ...
      if (document.file) {
         ...
         body.append('file_content', document.file.data);
         try {
            const response = await axios.post(url, body, header);
            document.link = response.data.link;
            document.file_type = response.data.file_type;
