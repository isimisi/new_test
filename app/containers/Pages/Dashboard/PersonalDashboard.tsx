...
import { getTags } from "@components/Tags/reducers/tagsActions";
...
function PersonalDashboard() {
  ...
  const messageNotif = useAppSelector((state) =>
    state.workspace.get("message")
  );
  ...
  const workspaces = useAppSelector((state) =>
    state.workspace.get("workspaces")
  ).toJS();
  ...
  const descriptionChange = (e: ChangeEvent<HTMLInputElement>) =>
    setWorkspacDescription(e.target.value);
  const [group, setGroup] = useState("");
  const changeGroup = (val: OnChangeValue<"" | SelectOptions, false>) =>
    setGroup(!val || typeof val === "string" ? "" : val.value);
  ...
  const changeCvrSearch = (val: OnChangeValue<SelectOptions, false>) =>
    setCvrSearch(val);
  const [countries, setCountries] = useState<MultiValue<SelectOptions>>([]);
  const handleChangeCountries = (values: OnChangeValue<SelectOptions, true>) =>
    setCountries(values || []);
  ...
  const createEmptyWorkspace = () => {
    if (plan_id === 1 && workspaces.length === 50) {
      dispatch(
        showNotifAction(t("personal-dashboard.can_not_create_more_workspaces"))
      );
    } else {
      user &&
        dispatch(
  ...
  const confirm = () => {
    if (cvrSearch && user) {
      const cvr =
        cvrSearch.value.includes("DK") && cvrSearch.value.length < 12
          ? cvrSearch.value.substring(2)
          : cvrSearch.value;
  ...
  const getAsyncOptions = (inputValue) =>
    axios
      .get(
        `${baseUrl}/workspaces/cvr/dropdown?q=${inputValue}&countries=${
          plan_id === 1
            ? JSON.stringify(["DK"])
            : countries.length > 0
  ...
  return (
    <div>
      ...
      <Grid container className={classes.root} spacing={3}>
        <Grid item xs={12}>
          {intro || removedIntro ? (
            <div className={classes.racoonAndText}>
              <div style={{ maxWidth: 500 }}>
                ...
                <Typography style={{ fontSize: 14 }}>
                  {dashboard === "structure"
                    ? t("personal-dashboard.type_description_structure")
                    : t("personal-dashboard.type_description_timeline")}
                </Typography>
              </div>
              <div
                style={{
            ...
            <Paper className={classes.introContainer} elevation={0}>
              <div>
                ...
                <Typography
                  component="p"
                  className={classes.description}
                  style={{ color: "white", marginBottom: 30, marginLeft: 2 }}
                >
                ...
                <div style={{ display: "flex" }}>
                  <Button
                    color="primary"
                    variant="contained"
                    onClick={() => clickOnIntroChoice("structure")}
                    style={{
                      marginRight: 7,
                  ...
                  <Button
                    color="primary"
                    variant="contained"
                    onClick={() => clickOnIntroChoice("timeline")}
                    style={{
                      marginRight: 7,
                      backgroundColor: "white",
                      color: "black"
          ...
          {intro || removedIntro
            ? dashboard === "structure" && (
              <Paper className={classes.papperBlock} elevation={0}>
                ...
                <section
                  className={classNames(classes.content, classes.whiteBg)}
                >
                  <AsyncSelect
                    styles={selectStyles()}
                    menuPlacement="auto"
                    ...
                    placeholder={t(
                      "workspaces.search_for_a_company_or_CVR_number"
                    )}
                    loadOptions={getAsyncOptions}
                    // eslint-disable-next-line react/no-unstable-nested-components
                    noOptionsMessage={() => (
                      <Typography>Her er intet at vise.</Typography>
                    )}
                  />
            ...
            : null}
          <Paper
            className={classes.papperBlock}
            elevation={0}
            style={{ position: "relative", paddingBottom: "59%" }}
          >
            <div className={classes.descBlock}>
              <div className={classes.titleText}>
                <Typography
        ...
        <Grid item md={!intro && !removedIntro ? 4 : 6} xs={12}>
          {!intro &&
          !removedIntro ? /** <PapperBlock title={t('personal-dashboard.how_to_start')} whiteBg /> */ null : (
              <PapperBlock
                title={
                  dashboard === "structure"
                    ? t("personal-dashboard.start_empty")
                    : t("personal-dashboard.start_empty_timeline")
                }
                whiteBg
                desc={
                  dashboard === "structure"
                    ? t("personal-dashboard.create_empty_workspace")
                    : t("personal-dashboard.create_empty_timeline")
                }
              >
                ...
                >
                  <CreatableSelect
                    ...
                    onChange={(newValue, _meta) =>
                      hanldeOnChange(newValue, _meta, changeTags, tags)}
                ...
                <div style={{ marginTop: theme.spacing(2) }}>
                  ...
                  >
                    {dashboard === "structure"
                      ...
                      : t("personal-dashboard.create_a_new_timeline")}
                  </Button>
                </div>
              </PapperBlock>
            )}
...
const openAction = (key: string, keyParent: string) => ({
  type: "OPEN_SUBMENU",
  key,
  keyParent
});
...
const Dash = connect(mapStateToProps, mapDispatchToProps)(PersonalDashboard);
