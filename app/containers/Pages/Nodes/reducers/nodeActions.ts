...
export const getNodes = (user: User) => async (dispatch) => {
...
export const postNode = (user: User, history: History) => async (dispatch) => {
  const url = `${baseUrl}/${NODES}`;
  const body = {};
  const header = authHeader(user);
  try {
...
export const showNode = (user: User, id: string) => async (dispatch) => {
...
export const deleteNode = (user: User, id: string, title: string) => async (dispatch) => {
  const url = `${baseUrl}/${NODES}/${id}`;
  const header = authHeader(user);
  ...
  try {
    ...
    dispatch(getNodes(user));
  } catch (error) {
    let message = genericErrorMessage;
    // @ts-ignore
    if (error?.response?.status === 403) {
      message =
        "Vi kunne ikke slette dit element, da du bruger det i andre sammenhænge";
    }
