...
import { NodeState, IImmutableNodeState } from "@customTypes/reducers/node";
...
const initialImmutableState: IImmutableNodeState = fromJS(initialState);
export default function reducer(
  ...
  switch (action.type) {
    ...
    case REMOVE_ATTRIBUT:
      return state.withMutations((mutableState) => {
        mutableState.update("attributes", (myList) =>
          myList.filter((v, i) => i !== action.index)
        );
        if (action.id) {
