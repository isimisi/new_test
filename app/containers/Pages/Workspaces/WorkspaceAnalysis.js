...
const WorkspaceAnalysis = () => {
  ...
  const save = () => {
    ...
    outputs.forEach((output) => {
      dispatch(
        saveAnalysis(id, output.action.output, JSON.stringify(output.elements))
      );
    });
  };
      ...
      {outputs.map((output, index) => (
        <Grid container className={classes.root}>
          ...
          {open && (
            <Grid
              item
              xs={open && !matchpattern.test(output.action.output) ? 2 : 0}
              style={{
                display: matchpattern.test(output.action.output) && "none",
              }}
            >
              <Hidden smDown>
                <Drawer
