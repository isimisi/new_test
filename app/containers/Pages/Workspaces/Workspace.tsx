...
import { reducer, proOptions, BASE_BG_GAP, BASE_BG_STROKE } from "./constants";
import {
  labelChange,
  descriptionChange,
  addGroup,
...
function Workspace(props) {
  ...
  const nodeElements: TCustomNode[] = useAppSelector((state) =>
    state[reducer].get("nodeElements")
  ).toJS();
  const edgeElements: TCustomEdge[] = useAppSelector((state) =>
  ...
  const initFunc = (_reactFlowInstance: ReactFlowInstance<any, any>) =>
    dispatch(
      showWorkspace(
        user,
        id as string,
  ...
  const { handleExcel, loadingExcel } = useExcelExport(
    nodeElements,
    edgeElements,
    label
  );
  ...
  const handleGetCompanyData = useCallback(
    (_id) => {
      dispatch(
        getCompanyData(
          user,
  ...
  const handleShare = (firstName, lastName, email, phone, edit) =>
    dispatch(
      shareWorkspace(
        user,
        id as string,
  ...
  return (
    <div style={{ cursor }}>
      ...
      />
      <div
        className={pub ? classes.pubRoot : classes.root}
      ...
      >
        <ReactFlow
          ...
          onNodeContextMenu={
            pub && !editable ? undefined : handleNodeContextMenu
          }
          onPaneContextMenu={
            pub && !editable ? undefined : handlePaneContextMenu
          }
          onSelectionContextMenu={
            pub && !editable ? undefined : handleSelctionContextMenu
          }
          onEdgeContextMenu={
            pub && !editable ? undefined : handleEdgeContextMenu
          }
