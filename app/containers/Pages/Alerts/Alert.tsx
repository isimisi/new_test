...
const Alert = () => {
   ...
   const conditions = useAppSelector((state) =>
      state[reducer].get('alertConditions')
   ).toJS();
   const conditionsDropDownOptions = useAppSelector((state) =>
      state[reducer].get('conditionsDropDownOptions')
   ).toJS();
   const tagOptions = useAppSelector((state) => state.tags.get('tags')).toJS();
   const tags = useAppSelector((state) =>
      state[reducer].get('alertTags')
   ).toJS();
   ...
   const onSave = () => {
      ...
      dispatch(
         putAlert(
            ...
            JSON.stringify(deletedConditions),
            JSON.stringify(tags)
         )
      );
   };
   ...
   const handleCreateOrSeeCondition = (condition, see) => {
      if (see) {
         window.open(
            '/app/conditions/' + encryptId(condition.condition_id),
            '_blank'
         );
      } else {
