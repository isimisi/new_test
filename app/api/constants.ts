...
export const baseUrl =
  window.location.hostname === "staging.juristic.io"
    ? "https://staging-api.juristic.io"
    : process.env.NODE_ENV === "production"
    ? "https://api.juristic.io"
    : window.location.hostname === "juristicfrontend.eu.ngrok.io"
    ? "https://juristicapi.eu.ngrok.io"
    : "http://localhost:3333";
...
export const genericErrorMessage =
  "Oops, it seems like we have some problems with our servers, try again later";
...
export const getId = (history: History) => {
  ...
  if (typeof id === "string") {
    return (
      CryptoJS.AES.decrypt(decodeURIComponent(id), "path")?.toString(CryptoJS.enc.Utf8) ||
      id
    );
  }
  return id;
};
export const getIdFromEncrypted = (id: string): string =>
  CryptoJS.AES.decrypt(decodeURIComponent(id), "path")?.toString(CryptoJS.enc.Utf8);
export const encryptId = (id: string): string =>
  id && encodeURIComponent(CryptoJS.AES.encrypt(id.toString(), "path").toString());
